...
study1_instruments <- list(
  ...
  post_namtpb = list(
    post_ascribed_responsibility = c("final_nam5", "final_nam6", "final_nam7", "final_nam8"),
    post_personal_norm = c("final_nam9", "final_nam10", "final_nam11", "final_nam12"),
    post_attitude = c("final_tpb1", "final_tpb2", "final_tpb3", "final_tpb4"),
    post_subjective_norm = c("final_tpb5", "final_tpb6", "final_tpb7", "final_tpb8"),
    post_behavioural_control = c("final_tpb9", "final_tpb10", "final_tpb11", "final_tpb12")),
  demo_openness = list(
...
study1_df_recoded <- study1_df_variance %>%
  ...
  mutate(across(.cols = c(age, caffeine, liking, income, education, politics, mailing),
         ...
         across(.cols = c(base_nam3, base_tpb11, perfo_nam3, docu_nam3, final_tpb11),
                .fns = ~ 8 - .x),
...
contrasts(study1_2long$medium_2) <- cbind(PerfvsFilm = c(-1, 1))
# attr(,"contrasts")
#             PerfvsFilm
# Film                -1
# Performance          1
...
contrasts(study1_2long$timepoint_2) <- cbind(EndvsInter = c(-1, 1))
# attr(,"contrasts")
#              EndvsInter
# Intermission         -1
# End                   1
...
# rename intention items to have meaningful names
study1_df <- study1_df %>%
  rename(base_intent_mailing = base_tpb13,
         base_intent_flyer = base_tpb14,
         base_intent_donation = base_tpb15,
...
# TODO: combine study 1 & 2
# TODO: wrangle into long df for plotting
```
...
Artists are inspired by society and contribute to it, often using their art to advocate for social justice.

As such, advocacy through art has become an object of psychological investigation, evaluating the effects on its audience. Media like documentary films, visual art, and songs have been proven to affect the aforementioned normative, empathic, and relational dimensions, and to mobilize behaviour in support of social justice causes. For example,

-   documentary film about wrongful imprisonment increased empathic responses towards prisoners, attitudes towards prison reform, and intention to support a petition regarding voting rights for people with felony convictions [@reddan2024]

-   TODO specify: documentaries about climate change raise awareness and change intention and behaviour regarding sustainability [@howell2011; @nolan2010; @sakellari2015]

-   TODO visual art: Pelowski & Specker; Pizzolante & Pelowski –\> is that narrative tho? Otherwise, stick with theatre. Or frame around naturalistic images (visual or auditory) vs. abstract (performance)

-   TODO specify lyrical content and type of helping behaviour: songs with prosocial lyrics prompt prosocial thoughts and empathic responses; empathy mediates effect on helping behaviour [@greitemeyer2009] & greitemeyer2009b; TODO read Greitemeyer 2011 review

-   tighten relationships: nature documentaries might elicit self-transcendent experiences that increase connection between viewer and nature, which has been suggested as driving factor for pro-environmental behaviour [@mccormack2021]. Watching documentary about a disabled abuse victim increased bonding with other audience members compared to neutral documentaries [@dunbar2016].

The advocacy through art that has been studied so far focuses on concrete narration and images to convey its message. However, not all art works on this level. Music beyond the song form, such as (contemporary) classical or folk music, is carried more by instruments and sounds and less by lyrics. Therefore, this kind of music has less tools for concrete narrative or imagery. Still, it is often intended to advocate for specific issues, such as TODO find more examples, maybe among Klezmer or Jazz?:
