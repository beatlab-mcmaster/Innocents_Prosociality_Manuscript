...
tab_model(intention_outcomes, study2_intention_outcomes, study2_intention_outcomes2, study2_intention_outcomes3, show.ci = FALSE, 
...
# Check correlations with control variables
dv_corr("ios_anna_z", control_vars, study2_dichotom, output = "table") # liking r = .36
...
dv_corr("target_empathic_concern_z", control_vars, study2_dichotom, output = "table") # age r = .3, liking r = .31
dv_corr("problem_awareness_z", control_vars, study2_dichotom, output = "table") # identity_lgbq r = .34
dv_corr("behavioural_intention_z", control_vars, study2_dichotom, output = "table") # liking r = .38
dv_corr("behavioural_intention_z", control_vars, subset(study2_dichotom, timepoint == "End"), output = "table") # liking r = .47
dv_corr("behavioural_intention_z", control_vars, subset(study2_dichotom, timepoint == "Baseline"), output = "table") # liking r = .33
dv_corr("sum_behaviour", control_vars, subset(study2_dichotom, timepoint == "End"), method = "spearman", output = "table") # liking r = .48; openness r = .33
...
# rownames(awareness_intercepts)[c(12, 171,  64, 145)] # p_id = c("sub-on2109", "sub-on295",  "sub-on2156", "sub-on271")
# 
# rm(awareness_intercepts)

s2_awareness_lmm <- lmer(problem_awareness_z ~ identity_lgbq_dich + group * timepoint + (1|p_id), data = subset(study2_long, !p_id %in% c("sub-on2109")))
# check_assumptions(s2_awareness_lmm, "problem_awareness_z", subset(study2_long, !p_id %in% c("sub-on2109"))) # removing one outlier fixed assumptions
...
# # Find the indices of the most extreme residuals
# order(abs(anna_empathy_residuals), decreasing = TRUE)[1:7]
# # Alternatively, plot outlier rows
# lattice::qqmath(anna_empathy_lmm, id=0.1, idLabels=~.obs)
# 
# rm(anna_empathy_residuals)

anna_empathy_lmm <- lmer(anna_empathic_concern_z ~ group * timepoint + (1|p_id), data = study2_long[-c(173, 174, 7, 60, 8, 59, 70), ])
check_assumptions(anna_empathy_lmm, "anna_empathic_concern_z", study2_long[-c(173, 174, 7, 60, 8, 59, 70), ]) # normality of intercepts still violated

# anna_empathy_intercepts <- ranef(anna_empathy_lmm)$p_id
# order(anna_empathy_intercepts$`(Intercept)`, decreasing = T)[1:2] # 25, 69
# rownames(anna_empathy_intercepts)[c(25, 69)] # "sub-on2120" "sub-on2160"
# which(study2_long$p_id %in% c("sub-on2120", "sub-on2160")) # 239 240 319 320
# 
# rm(anna_empathy_intercepts)
# 
# anna_empathy_lmm <- lmer(anna_empathic_concern_z ~ group * timepoint + (1|p_id), data = study2_long[-c(173, 174, 7, 60, 8, 59, 70, 239, 240, 319, 320), ])
# check_assumptions(anna_empathy_lmm, "anna_empathic_concern_z", study2_long[-c(173, 174, 7, 60, 8, 59, 70, 239, 240, 319, 320), ]) # normality of intercepts STILL violated
...
target_empathy_lmm <- lmer(target_empathic_concern_z ~ age_z + liking_z + group * timepoint + (1|p_id), data = study2_long)
...
# order(target_empathy_intercepts$`(Intercept)`, decreasing = T)[1:4] # 40, 115, 99, 144
# rownames(target_empathy_intercepts)[c(40, 115, 99, 144)] # "sub-on2139", "sub-on260", "sub-on242", "sub-on29" 
# 
# target_empathy_lmm <- lmer(target_empathic_concern_z ~ age_z + liking_z + group * timepoint + (1|p_id), data = subset(study2_long, !p_id %in% c("sub-on2139", "sub-on260", "sub-on242", "sub-on29")))
# check_assumptions(target_empathy_lmm, "target_empathic_concern_z", subset(study2_long, !p_id %in% c("sub-on2139", "sub-on260", "sub-on242", "sub-on29"))) # normality of intercepts STILL violated
...
# anna_closeness_residuals[c(29, 30, 57, 63, 58, 64)] # indices: 31, 32, 63, 69, 64, 70
# 
# rm(anna_closeness_residuals)
# 
# anna_closeness_lmm <- lmer(ios_anna_z ~ liking_z + group * timepoint + (1|p_id), data = study2_long[-c(31, 32, 63, 69, 64, 70), ])
# check_assumptions(anna_closeness_lmm, "ios_anna_z", study2_long[-c(31, 32, 63, 69, 64, 70), ]) # residual outliers are gone, but assumptions are still violated
...
check_assumptions(target_closeness_lmm, "ios_target_z", study2_long) # residuals & intercepts violate normality, though intercept q-q looks good to me...
...
# check_assumptions(s2_intention_lmm, "behavioural_intention_z", study2_long) # non-normal residuals

# # Extract and standardize residuals
# intention_residuals <- resid(s2_intention_lmm, type = "pearson")
# # Find the most extreme residuals
# order(abs(intention_residuals), decreasing = T)[1:4] # 149 150 297 298
# # look up their indices
# intention_residuals[c(149, 150, 297, 298)] # 149       150       297       298
# 
# rm(intention_residuals)

s2_intention_lmm <- lmer(behavioural_intention_z ~ liking_z + group * timepoint + (1|p_id), data = study2_long[-c(149, 150, 297, 298), ])
# check_assumptions(s2_intention_lmm, "behavioural_intention_z", study2_long[-c(149, 150, 297, 298), ]) # now, residuals are normal

# # extract intercepts
# intention_intercepts <- ranef(s2_intention_lmm)$p_id
# # find p_ids with largest positive and negative intercepts
# order(intention_intercepts$`(Intercept)`, decreasing = T)[1] # 143
# order(intention_intercepts$`(Intercept)`, decreasing = F)[1] # 145
# rownames(intention_intercepts)[c(143, 145)] # "sub-on27"   "sub-on271"
# # find indices of those p_ids in original data
# which(study2_long$p_id %in% c("sub-on27", "sub-on271")) # 13  14 141 142
# 
# s2_intention_lmm <- lmer(behavioural_intention_z ~ group * timepoint + (1|p_id), data = study2_long[-c(149, 150, 297, 298, 13, 14, 141, 142), ])
# check_assumptions(s2_intention_lmm, "behavioural_intention_z", study2_long[-c(149, 150, 297, 298, 13, 14, 141, 142), ]) # exclusion did not help

summary(s2_intention_lmm)
my_effectsize(s2_intention_lmm)
```

As opposed to study 1, openness and political orientation had associations smaller than $r = .3$ with problem awareness and were not included as covariates in the model. However, participants who self-identified with a marginalized sexual orientation tended to have a higher problem awareness (`r apa_print(cor.test(study2_long$problem_awareness_z, study2_long$identity_lgbq_dich, method = "spearman"))$full_result`); hence this variable was controlled for. Consistent with study 1, the model $Awareness \sim LGBQ + Stimulus * Time + (1|Participant)$ yielded the hypothesized small effect of time `r eval_fixed(s2_awareness_lmm, "timepointPostvsPre")`, with participants reporting higher problem awareness after the event (`r print_descr(study2_df$post_problem_awareness)`) than before (`r print_descr(study2_df$base_problem_awareness)`; see @fig-awareness). The stimuli did not have a significant main effect or interaction with time.

### Empathy

We fitted two LMMs for empathy with Anna Vasquez and with a hypothetical victim of wrongful imprisonment separately. As expected, empathy with Anna Vasquez showed a small increase from baseline (`r print_descr(study2_df$base_anna_empathic_concern)`) to after the event (`r print_descr(study2_df$post_anna_empathic_concern)`), `r eval_fixed(anna_empathy_lmm, "timepointPostvsPre")` (see @fig-empathy), but the interaction with the stimuli from study 1 did not replicate, nor did the stimuli have a significant main effect. Empathy with the hypothetical victim was associated with older age (`r apa_print(cor.test(study2_long$target_empathic_concern_z, study2_long$age, method = "spearman"))$full_result`) and with liking contemporary percussion music (`r apa_print(cor.test(study2_long$target_empathic_concern_z, study2_long$liking, method = "spearman"))$full_result`), hence these variable were included as covariates in the model. Empathy with the hypothetical victim showed a very small increase from baseline (`r print_descr(study2_df$base_target_empathic_concern)`) to after the event (`r print_descr(study2_df$post_target_empathic_concern)`), `r eval_fixed(target_empathy_lmm, "timepointPostvsPre")` (see @fig-empathy-2), whereas the main and interaction effects of the stimuli were not significant.
