...
# anna_ios_lineplot <- plot_meanSE(df = combined_long, dv = "ios_anna", facet_var = "facet", ylab = "Rating", my_colours = c("#41AB5D", "#A1D99B"))
  ...
  # TODO dodge by group: add filmfirst and perffirst as separate geoms
...
combined_long %>%
  mutate(timepoint = case_when(
    (study == 2 & !medium == "Baseline") ~ "Intermission",
    .default = timepoint
  ),
...
combined_long %>%
  ...
  mutate(
    ...
    (study == 1 & timepoint == "End") ~ as.numeric(order) + .3,
    (study == 2 & timepoint == "End") ~ as.numeric(order)
  )) %>%
  ungroup() %>%
  ggplot(aes(x = order,
             y = anna_empathic_concern,
...
n_behaviour <- combined_long %>%
  rowwise() %>%
  mutate(completed = ifelse(study == 2, 1, completed)) %>% # for study 2, set completed to TRUE
...
# 
# behaviour_annotation <-  annotation_custom2(rasterGrob(icon_behav, interpolate = TRUE), xmin = 2.2, 2.9, 69, 75, data = subset(behaviour_plot_df, facet == "1:In-person"))
# 
