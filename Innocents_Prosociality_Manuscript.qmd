...
#| fig-cap: "Mean and CI of closeness with Anna Vasquez across the event by time point (x-axis) and order of the media (line style), split for the in-person (left) and livestream (middle) audience of study 1, and the audience of study 2 (right). Closeness was rated on a visual analog scale ranging from 1 (0% overlap) to 7 (90% overlap)."
#| echo: false
...
closeness_lineplot <- combined_long %>%
    ...
    #            alpha = 0.4,
    #            position = position_jitterdodge(jitter.width = .2, jitter.height = .1, dodge.width = 0.5)) +
    geom_line(aes(group = p_id),
              color = "darkgrey",
               alpha = 0.4,
               position = position_jitterdodge(jitter.width = .2, jitter.height = .1, dodge.width = 0.5)) +
    # TODO look into this ggnewscale::new_scale_color() +
