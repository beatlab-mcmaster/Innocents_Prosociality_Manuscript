...
Opening paragraph: takeaways from the subjective experience paragraph? + details & figures in supplemental

### Effect of Event Conditions

We evaluated the effects of the event conditions on *awareness* of wrongful imprisonment, *empathy* with the wrongfully imprisoned Anna Vasquez, and *closeness* with Anna Vasquez (Hypotheses 1a, 2a, and 3a). 
...
# awareness_lmm <- lmer(problem_awareness_z ~ demo_openness_z + base_problem_awareness_z + attendance * medium * timepoint + (1|p_id), data = study1_2long)
...
# 
# awareness_log <- lmer(problem_awareness_log ~ demo_openness_z + base_problem_awareness_z + attendance * medium * timepoint + (1|p_id), data = study1_2long)
...
# 
# awareness_recpr <- lmer(problem_awareness_recpr ~ demo_openness_z + base_problem_awareness_z + attendance * medium * timepoint + (1|p_id), data = study1_2long)
...
awareness_lmm <- lmer(problem_awareness_z ~ demo_openness_z + base_problem_awareness_z + attendance * medium * timepoint + (1|p_id), data = subset(study1_2long, !(p_id %in% c("sub-on107", "sub-in028")) & !is.na(problem_awareness_z)))
...
awareness_cov <- lmer(problem_awareness_z ~ demo_openness_z + base_problem_awareness_z + (1|p_id), data = subset(study1_2long, !(p_id %in% c("sub-on107", "sub-in028")) & !is.na(problem_awareness_z)))
```
...
#| apa-note: Mean and CI of awareness by stimulus (x-axis), split for the in-person (left) and livestream (middle) audience of study 1, and the stream audience of study 2 (right). Awareness was rated on a scale ranging from 1 (strongly disagree) to 7 (strongly agree). Significance levels refer to one-sided pairwise t-tests. &ast;$p < .05$, &ast;&ast;$p < .01$, &ast;&ast;&ast;$p < .001$, n.s. = not significant.
#| fig-height: 2.5
#| echo: false

# TODO Figure out how to add sample size.

fig_awareness_study1 <- plot_meanSE(df = subset(combined_long, study == "Study 1"), dv = "problem_awareness", ylab = "Problem Awareness", xlab = "Stimulus") + facet_wrap(vars(facet), scales = "free")

fig_awareness_study2 <- plot_meanSE(df = subset(combined_long, study == "Study 2"), dv = "problem_awareness", ylab = "Problem Awareness", xlab = "") + facet_wrap(vars(facet), scales = "free") 

ggpubr::ggarrange(
        fig_awareness_study1,
        fig_awareness_study2 + theme(axis.title.y = element_blank()),
        nrow = 1, 
        ncol = 2,
        widths = c(2.2, 1),
        legend = "bottom",
...
Audiences already had a high problem awareness before the event `r print_descr(outcomes$problem_awareness_Baseline)`, which increased slightly at intermission `r print_descr(outcomes$problem_awareness_Intermission)`, and slightly further at the end of the event `r print_descr(outcomes$problem_awareness_End)` (see @fig-awareness). Openness to experience was moderately associated with higher problem awareness, `r apa_print(cor.test(study1_3long$problem_awareness, study1_3long$demo_openness, method = "spearman"))$full_result` and was therefore included as a covariate in the LMM, resulting in the formula $Awareness \sim Openness + Baseline + Attendance * Stimulus * Time + (1|Participant)$. For all three outcomes (awareness, empathy, and closeness), the baseline significantly predicted the outcome, but the data were significantly more likely under the full model including the event conditions, compared to a model with just the covariates and the random intercept (see @apx-modelcomps). 

The main effect and interactions involving the stimulus were not significant, indicating no difference between performance and film. Thus, the data did not support Hypothesis 3. The only significant fixed effect was a small main effect of time, `r eval_fixed(awareness_lmm, "timepointEndvsInter")`, with higher problem awareness at the end of the event compared to intermission, regardless of stimulus and attendance mode. An additional post-hoc test revealed that the increase from baseline to intermission was also small but significant, `r apa_print(pa_1sthalf)$full_result`, $d = `r printnum(pa_1sthalf_d$estimate)`$.
...
#| fig-cap: Changes in Closeness with Anna Vasquez Throughout the Event 
#| apa-note: Mean and CI of closeness by time point (x-axis) and order of the media (line style), split for the in-person (left) and livestream (middle) audience of study 1, and study 2 (right). Closeness was rated on a visual analog scale ranging from 1 (0% overlap) to 7 (90% overlap). Significance levels refer to one-sided pairwise t-tests. &ast;$p < .05$, &ast;&ast;$p < .01$, &ast;&ast;&ast;$p < .001$, n.s. = not significant.
#| fig-height: 2.5
#| echo: false

# anna_ios_lineplot <- plot_meanSE(df = combined_long, dv = "ios_anna", facet_var = "facet", ylab = "Rating", my_colours = c("#41AB5D", "#A1D99B"))

  # annotation_custom2(rasterGrob(icon_close, interpolate = TRUE), # add icon
  #                   xmin = 0.6, 1.5, 6.4, 7.2,
  #                   data = subset(combined_long, facet == "Study 1 Livestream"))
...
study1_3long %>%
  # manually dodge x-axis by group: https://stackoverflow.com/a/44657850
  mutate(dodge_timepoint = ifelse(order == "Film first", 
                                  as.numeric(timepoint) - .1,
                                  as.numeric(timepoint) + .1)) %>%
  ggplot(aes(x = timepoint,
             ...
             shape = medium)) +
  geom_blank() +
    geom_point(aes(x = dodge_timepoint),
               alpha = 1) +
    geom_line(aes(x = dodge_timepoint, group = p_id),
              alpha = 1) +
    scale_colour_manual(values = c("#41AB5D", "#A1D99B"), guide = "none") +
    # new_scale_color() +
  ...
  facet_wrap(vars(attendance), scales = "free_x") +
...
target_ios_lineplot <- plot_meanSE(df = combined_long, dv = "ios_target", facet_var = "facet", ylab = "Rating", my_colours = c("#41AB5D", "#A1D99B")) +
...
```{r}
#| label: fig-manualdodge

# TODO change mean+CI to boxplots
...
study1_3long %>%
  ...
  ggplot(aes(x = timepoint,
             ...
             shape = medium)) +
  geom_blank() +
    geom_boxplot(aes(x = dodge_timepoint, group = dodge_timepoint),
                 outlier.shape = NA,
                 color = "black",
                 linetype = "solid",
                 alpha = .5) +
    ...
    geom_line(aes(x = dodge_timepoint, group = p_id),
              alpha = 1) +
    scale_colour_manual(values = c("dodgerblue", "darkorange"), guide = "none") +
    scale_fill_manual(values = c("dodgerblue", "darkorange"), guide = "none") +
    # new_scale_color() +
    # stat_summary(color = "black",
    #              fun = "mean", # connect means
    #              geom = "line",
...
study1_3long %>%
  ...
  geom_rain(alpha = .8,
            rain.side = "f",
            id.long.var = "p_id",
            boxplot.args = list(color = "black", alpha = .5, outlier.shape = NA),
            violin.args = list(color = NA, alpha = .5),
            boxplot.args.pos = list(width = .1,
             position = ggpp::position_dodgenudge(width = .1,
                                                  x = rep(c(-.13, -.13, # Baseline F-P, Baseline P-F
                                                        -.13, .13, 
                                                         .13, .13), 2))),
            violin.args.pos = list(width = .7,
             position = position_nudge(x = (c(rep(-.2, 256*2), rep(-.2, 256*2),# Baseline F-P
                                             rep(-.2, 256*2), rep(-.2, 256*2), # Baseline P-F
                                             rep(-.2, 256*2), rep(-.2, 256*2), # Intermission F-P
                                             rep(.2, 256*2), rep(.2, 256*2), # Intermission P-F
                                             rep(.2, 256*2), rep(.2, 256*2), # End F-P
  ...
  stat_summary(fun = mean, 
               ...
               color = "black",
               linewidth = 0.9,
               aes(group = order, linetype = order)) +
  stat_summary(fun = mean, 
               geom = "point", 
               color = "black",
               size = 2.5,
               aes(group = order, shape = medium)) +
  ...
  guides(
      linetype = guide_legend("Order"),
      colour = guide_legend("Order"),
      shape = "none",
      fill = "none"
    ) +
  facet_wrap(vars(attendance))

# try without violin

combined_long %>%
  mutate(timepoint = case_when(
    (study == "Study 2" & !medium == "Baseline") ~ "Intermission",
    .default = timepoint
  ),
  timepoint = factor(timepoint, levels = c("Baseline", "Intermission", "End"))) %>%
  ggplot(aes(x = timepoint,
             y = anna_empathic_concern,
             fill = order,
             color = order,
  ...
  geom_rain(alpha = .8,
            ...
            boxplot.args.pos = list(width = .3,
             position = ggpp::position_dodgenudge(width = .1,
                                                  x = c(-.13, .13, -.13, .13, -.13, .13, # Baseline
                                                        -.13, .13, -.13, .13, -.13, .13, 
                                                         -.13, .13, -.13, .13)))
            ) +
  theme_bw() +
...
combined_long %>%
  ...
  ggplot(aes(x = order,
    ...
    geom_line(aes(x = dodge_order, group = p_id),
              color = "darkgrey",
              alpha = .7) +
    stat_summary(aes(x = dodge_order, group = order),
                 color = "black",
                 alpha = .7,
                 fun = "mean", # connect means
                 geom = "line",
    ...
    stat_summary(aes(x = dodge_order),
                 ...
                 show.legend = FALSE) +
    guides(
      shape = guide_legend("Measurement"),
      color = guide_legend("Measurement")
    ) +
...
```{r}
#| label: fig-empathy-plot
#| fig-cap: Changes in Empathic concern with Anna Vasquez Throughout the Event 
#| apa-note: Mean and CI of empathy by time point (x-axis) and order of the media (line style), split for the in-person (left) and livestream (middle) audience of study 1, and study 2 (right). Empathy was rated on a scale ranging from 1 (not at all) to 7 (extremely). Significance levels refer to one-sided pairwise t-tests. &ast;$p < .05$, &ast;&ast;$p < .01$, &ast;&ast;&ast;$p < .001$, n.s. = not significant.
#| echo: false
...
empathy_lineplot <- plot_meanSE(df = combined_long, dv = "anna_empathic_concern", facet_var = "facet", ylab = "Rating", my_colours = c("#DC267F", "#D4ABB7")) +
  theme(legend.position = "top", # move legend below plot
        legend.justification.top = "right",
        legend.direction = "horizontal")

print(empathy_lineplot)

annotate_figure(empathy_lineplot, top = rasterGrob(icon_empath, x = 0.1, y = -0.5, width = 0.07, interpolate = TRUE))

# TODO 2:Livestream panel: add a third x-axis point so that lines are same length as in other panels?
# TODO arrange hyothetical target into the same pane
```

```{r}
#| label: fig-target-plot
#| fig-cap: Changes in Empathic Concern with a Hypothetical Prisoner
#| fig-height: 4.6
#| fig-width: 2.9

plot_meanSE(df = combined_long, dv = "target_empathic_concern", facet_var = "facet", ylab = "Rating", my_colours = c("#DC267F", "#D4ABB7")) +
  theme(legend.position = "top", # move legend below plot
        legend.justification.top = "right",
        legend.direction = "horizontal")
```
...
```{r}
#| label: fig-awareness-plot
#| fig-cap: Changes in Problem Awareness Throughout the Event 
#| apa-note: Mean and CI of awareness by time point (x-axis) and order of the media (line style), split for the in-person (left) and livestream (middle) audience of study 1, and study 2 (right). Awareness was rated on a scale ranging from 1 (strongly disagree) to 7 (strongly agree). Significance levels refer to one-sided pairwise t-tests. &ast;$p < .05$, &ast;&ast;$p < .01$, &ast;&ast;&ast;$p < .001$, n.s. = not significant.
#| echo: false

awareness_lineplot <- plot_meanSE(df = combined_long, dv = "problem_awareness", facet_var = "facet", ylab = "Rating", my_colours = c("#88ccee", "#B9CEE4")) +
  theme(legend.position = "bottom", 
        legend.direction = "horizontal")

print(awareness_lineplot)
```

The only correlation reaching that threshold was between problem awareness and openness to experience, `r apa_print(cor.test(study1_2long$problem_awareness_z, study1_2long$demo_openness, method = "spearman"))$full_result`. Therefore, the problem awareness model includes openness as an additional covariate.

```{r}
#| label: fig-behaviour-count
#| fig-cap: Proportion of Supportive Behaviours Observed in Each Sample
#| apa-note: TODO
#| echo: false
...
n_behaviour <- combined_long %>%
  rowwise() %>%
  mutate(completed = ifelse(study == "Study 2", 1, completed)) %>% # for study 2, set completed to TRUE
  filter(completed == 1) %>% # only use full datasets of study 1
  filter(timepoint == "End") %>% # only use End data
  dplyr::select(flyer, mailing, donation) %>%
  pivot_longer(cols = c(flyer, mailing, donation), names_to = "behaviour", values_to = "response") %>%
...
# TODO change to ordinal significance test rather than chisq
# count how many participants performed at least one behaviour by attendance mode and test for significance
# behaviour_by_attendance <- table(study1_df$attendance, study1_df$behaviour_dich)
...
# behaviour_by_order <- table(study1_df$order, study1_df$behaviour_dich)
...
behaviour_plot_df <- combined_long %>%
  filter(is.na(completed) | completed == 1) %>%
  dplyr::select(p_id, facet, flyer, mailing, donation) %>%
  distinct(.keep_all = TRUE) %>%
  pivot_longer(cols = c(flyer, mailing, donation), names_to = "behaviour", values_to = "response") %>%
  group_by(behaviour, facet, response) %>%
  summarize(count = n(), .groups = "drop") %>%
  group_by(behaviour, facet) %>%
  mutate(perc = round(count / sum(count) *100), digits = 1) %>%
  mutate(response = replace(response, is.na(response), -9),
         response = factor(response, levels = c(-9, 0, 1), labels = c("Not applicable", "No", "Yes")),
         behaviour = factor(behaviour, levels = c("flyer", "mailing", "donation"), labels = c("Flyer", "Mailing", "Donation")))
...
# 
# behaviour_annotation <-  annotation_custom2(rasterGrob(icon_behav, interpolate = TRUE), xmin = 2.2, 2.9, 69, 75, data = subset(behaviour_plot_df, facet == "Study 1 In-person"))
# 
...
study1_behav <- study1_2long %>%
  ...
  filter(!is.na(sum_behaviour_inclNA_fact)) # remove NAs
...
sumbehav_step1 <- MASS::polr(sum_behaviour_fact ~ ios_anna_z + anna_empathic_concern_z + problem_awareness_z, data = study1_behav, Hess = TRUE)

# Step 2: behaviour ~ closeness + empathy + awareness + intention
sumbehav_step2 <- MASS::polr(sum_behaviour_fact ~ ios_anna_z + anna_empathic_concern_z + problem_awareness_z + behavioural_intention_z, data = study1_behav, Hess = TRUE)
...
r2_nagelkerke(sumbehav_step1)
r2_nagelkerke(sumbehav_step2)
```
...
TODO row 21, 29, 35, 36, 38, 47, 50, 54 guessed Hypothesis 1, exclude from that analysis.

Run regression of intention twice: once for the film, once for the performance?
...
To cite an appendix as a whole, reference it with the `@apx-` prefix.
...
model_comp_table <- matrix(ncol = 2, nrow = 0) %>% # initiate table structure
  ...
  rbind(c("  Full Model", "Awareness \\textasciitilde Openness + Baseline + Attendance * Stimulus * Time + (1|Participant)")) %>%
