...
# TODO define mapping of variables to constructs

# within participants, count unique values across the items making up one construct
count_unique_values <- function(data, instruments) {
  # @data (data.frame, required): dataframe holding the items specified in the list of constucts
  # @instruments (list, required): list of instruments and the items each constructis made up of
  df_variance <- data # copy dataframe
  
  for (instrument in names(instruments)) {
    items <- instruments[[instrument]] # item columns
...
study1_variance <- count_unique_values(study1_df, study1_constructs)



#### cALCULATE SCALE SCORES IF AT LEAST 80% OF ITEMS HAVE BEEN ANSWERED AND IF THERE IS SUFFICIENT VARIANCE ###

calculate_scale_score <- function(scales) {
  n_items <- length(items)
  valid_items <- sum(!is.na(items))
  threshold <- 0.8 * n_items
  
  if (valid_items >= threshold) {
...
# define mapping of variables to instruments
study2_instruments <- list(
  "base_target_empathy" = grep("base_target_", colnames(study2_df_deident), value = TRUE),
  "base_ios" = c("base_ios_allen", "base_ios_john", "base_ios_anna", "base_ios_inperson", "base_ios_remote"),
  "base_anna_empathy" = grep("base_anna_", colnames(study2_df_deident), value = TRUE),
  "base_namtpb" = c(grep("base_nam", colnames(study2_df_deident), value = TRUE), grep("base_tpb", colnames(study2_df_deident), value = TRUE)), 
  ...
  "post_target_empathy" = grep("post_target_", colnames(study2_df_deident), value = TRUE),
  "post_ios" = c("post_ios_allen", "post_ios_john", "post_ios_anna", "post_ios_inperson", "post_ios_remote"),
  "post_anna_empathy" = grep("post_anna_", colnames(study2_df_deident), value = TRUE),
  "post_namtpb" = c(grep("post_nam", colnames(study2_df_deident), value = TRUE), grep("post_tpb", colnames(study2_df_deident), value = TRUE)), 
  "post_presence" = grep("_presence", colnames(study2_df_deident), value = TRUE),
  "demo_openness" = grep("open", colnames(study2_df_deident), value = TRUE)
)

# count unique values within participants within instruments
study2_df_variance <- count_unique_values(study2_df_deident, study2_instruments)

# across sample, count ratio of instruments that lack variances
total_instrument_observations <- 0
novariance_observations <- 0
for (instrument in names(study2_instruments)) {
  cutoff <- ifelse(length(items) > 10, 2, 1)
  
  total <- sum(!is.na(study2_df_variance[[paste0(instrument, "_unique")]])) # count total observations per instrument
  novariance <- sum(study2_df_variance[[paste0(instrument, "_unique")]] <= cutoff, na.rm = TRUE) # count cases where unique values are below cutoff
  
  total_instrument_observations = total_instrument_observations + total
  novariance_observations = novariance_observations + novariance
} 

rm(study2_instruments, total, novariance, study2_df_deident)
...
# Define mapping of items to scales
study2_scales <- list(
  ...
  "physical_presence" = c("physical_presence1", "physical_presence2", "physical_presence3", "physical_presence4"),
  "social_presence" = c("social_presence1", "social_presence2", "social_presence3"),
  "trait_openess" = c("open1", "open4", "open6", "open8", "open11", "open12")
)


test_listoflists <- list(
  "instrument1" = list(scales = list("scale1" = c("item1", "item2"), "scale2" = c("item3", "item4")), unique_col = "instrument1_unique")
)
```
